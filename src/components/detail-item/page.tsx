...
export default function DetailItem() {
  ...
  const handleGenerateLabel = () => {
    toast.info("Fitur generate label akan segera hadir");
  };

  const handleGenerateSmallLabel = () => {
    toast.info("Fitur generate small label akan segera hadir");
  };
  ...
  return (
    <div className="w-full lg:max-w-7xl md:max-w-2xl max-w-md mx-auto">
      <div className="rounded-2xl border border-gray-200/50 bg-white/80 shadow-sm backdrop-blur-sm dark:border-white/5 dark:bg-white/5">
        {/* Search Bar & Generate Label Buttons */}
        <div className="border-b border-gray-200/50 p-6 dark:border-white/5">
          <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button
                onClick={handleGenerateSmallLabel}
                variant="outline"
                ...
                <TableRow className="border-b border-gray-200/50 dark:border-white/5">
                  {tableHeaders.map((header, index) => (
                    <TableCell
                      key={index}
                      isHeader
                      className={`bg-gradient-to-br from-gray-50 to-gray-100/50 px-2 py-4 text-left text-[clamp(2px,0.85rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300 ${
                        header.className || ""
                      }`}
                    >
                      {header.label}
