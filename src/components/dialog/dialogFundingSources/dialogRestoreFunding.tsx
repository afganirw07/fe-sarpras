...
import { restoreDeletedFundingSource } from "@/lib/funding-sources";
...
export default function RestoreFundingSource({
  fundingSourceId,
  onSuccess,
}: Props) {
  ...
  async function handleRestore(e: React.MouseEvent) {
    ...
    if (!fundingSourceId) {
      toast.error("ID sumber dana tidak ditemukan");
      return;
    }

    try {
      setLoading(true);
     await restoreDeletedFundingSource(fundingSourceId);

      toast.success("Sumber dana berhasil dipulihkan");
      onSuccess?.();
    } catch (err: any) {
      toast.error(err.message || "Gagal memulihkan sumber dana");
    } finally {
      setLoading(false);
    }
  }
