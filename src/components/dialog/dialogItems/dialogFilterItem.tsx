...
export default function DialogFilterItems({
  ...
  const handleReset = () => setLocal({ condition: "", period: null });
  ...
  const activeCount = (local.condition ? 1 : 0) + (local.period ? 1 : 0);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-md dark:bg-black">
        ...
        <div className="mt-4 flex flex-col gap-6">
          ...
          <div className="grid gap-3">
            <Label className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
              Periode Waktu <span className="normal-case font-normal text-gray-400">(berdasarkan updated_at)</span>
            </Label>
            <div className="flex flex-wrap gap-2">
              {PERIODS.map((p) => (
                <button
