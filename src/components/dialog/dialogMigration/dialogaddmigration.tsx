...
import { createMigration, type DetailItem } from "@/lib/migration";
...
/**
 * letter_status default saat create.
 * Sesuaikan dengan value yang dipakai di sistem kamu.
 */
const DEFAULT_LETTER_STATUS = "pending";

// ─── Main Component ──────────────────────────────────────────────────────────

export default function DialogAddMigration({
  onSuccess,
}: {
  onSuccess?: () => void;
}) {
  ...
  // ── Room name lookup (untuk kolom "WH Asal" di tabel kanan) ──
  ...
  // ── Remove item from staged → stagedIds change triggers re-fetch left ──
                  ...
                  {paginatedRight.length === 0 ? (
                    ...
                    paginatedRight.map((item, index) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="border px-4 py-4 text-sm text-gray-500">
                          {/* room_id → lookup nama dari state rooms */}
