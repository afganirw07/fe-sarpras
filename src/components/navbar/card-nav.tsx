...
const CardNav: React.FC<CardNavProps> = ({
  ...
  const buttonMobileRef = useRef<HTMLButtonElement | null>(null);
  ...
  const createTimeline = () => {
    ...
    if (buttonMobileRef.current) {
      gsap.set(buttonMobileRef.current, { y: 50, opacity: 0 });
    }
    ...
    if (buttonMobileRef.current) {
      tl.to(buttonMobileRef.current, { y: 0, opacity: 1, duration: 0.4, ease }, '-=0.32');
    }
  ...
  return (
    <div
      className={`card-nav-container fixed inset-x-0 mx-auto w-[90%] max-w-200 z-99 top-[1.2em] md:top-[2em] ${className}`}
    >
      <nav
        ref={navRef}
