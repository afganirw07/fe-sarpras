...
export default function TableKategori({
  ...
  onStatsUpdate?: (stats: KategoriStats) => void;
}) {
  ...
  const [totalAllSubkategori, setTotalAllSubkategori] = useState(0);
  ...
  const fetchAllData = async (page = 1) => {
    try {
      ...
      setTotalPages(catsRes.pagination.totalPages);
      setTotalData(catsRes.pagination.total);
      setTotalAllSubkategori(subsRes.data.length); 

      const categoryIdsOnPage = new Set(catsRes.data.map((c) => c.id));
      const subsOnPage = subsRes.data.filter((s) =>
        categoryIdsOnPage.has(s.category_id)
      );

      onStatsUpdate?.({
        totalKategori: catsRes.pagination.total,
        totalSubkategori: subsOnPage.length, 
        totalHasilPencarian: catsRes.data.length,
      });
    } catch {
  ...
  useEffect(() => {
    ...
    const subsFiltered = subcategories.filter((s) =>
      filteredCategoryIds.has(s.category_id)
    );

    onStatsUpdate?.({
      totalKategori: totalData,
      totalSubkategori: subsFiltered.length, 
      totalHasilPencarian: filteredCategories.length,
    });
  }, [filteredCategories, totalData, subcategories]);
            ...
            <Table className="w-full">
              <TableHeader>
                <TableRow className="border-b border-gray-200/50 dark:border-white/5">
                  <TableCell isHeader className="bg-linear-to-br w-20 from-gray-50 to-gray-100/50 px-[clamp(12px,1vw,20px)] py-[clamp(10px,0.9vw,16px)] text-center text-[clamp(10px,0.7rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300">No</TableCell>
                  <TableCell isHeader className="bg-linear-to-br from-gray-50 to-gray-100/50 px-[clamp(12px,1vw,20px)] py-[clamp(10px,0.9vw,16px)] text-left text-[clamp(10px,0.7rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300">Kode</TableCell>
                  <TableCell isHeader className="bg-linear-to-br from-gray-50 to-gray-100/50 px-[clamp(12px,1vw,20px)] py-[clamp(10px,0.9vw,16px)] text-left text-[clamp(10px,0.7rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300">Nama Kategori</TableCell>
                  <TableCell isHeader className="bg-linear-to-br from-gray-50 to-gray-100/50 px-[clamp(12px,1vw,20px)] py-[clamp(10px,0.9vw,16px)] text-left text-[clamp(10px,0.7rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300">Subkategori</TableCell>
                  <TableCell isHeader className="bg-linear-to-br w-32 from-gray-50 to-gray-100/50 px-[clamp(12px,1vw,20px)] py-[clamp(10px,0.9vw,16px)] text-center text-[clamp(10px,0.7rem,12px)] font-semibold uppercase tracking-wider text-gray-700 dark:from-white/5 dark:to-white/10 dark:text-gray-300">Aksi</TableCell>
                </TableRow>
              </TableHeader>
                        ...
                        {(() => {
                          ...
                          return subs.length ? (
                            <div className="space-y-1.5">
                              {subs.map((sub) => (
                                <div key={sub.id} className="bg-linear-to-r flex items-center gap-2 rounded-lg from-blue-50 to-purple-50 px-3 py-2 dark:from-blue-900/20 dark:to-purple-900/20">
                                  <span className="text-[clamp(9px,0.65rem,11px)] font-medium text-blue-900 dark:text-blue-300">{sub.name}</span>
                                  <span className="ml-auto rounded bg-blue-200/50 px-2 py-0.5 text-[clamp(9px,0.65rem,11px)] font-semibold text-blue-700 dark:bg-blue-800/30 dark:text-blue-400">{sub.code}</span>
                                </div>
                              ))}
                            </div>
                          ) : (
                            <div className="flex items-center justify-center rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/5">
                              <span className="text-[clamp(11px,0.85rem,14px)] text-gray-400">Belum ada subkategori</span>
                            </div>
                          );
                        })()}
                      </TableCell>
            ...
            <div className="flex items-center justify-between border-t px-4 py-3">
              <div className="text-muted-foreground flex items-center gap-4 text-sm">
                <span>
                  ...
                  {Math.min(currentPage * perPage, totalData)} of {totalData} rows
                </span>
