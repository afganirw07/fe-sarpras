...
export default function TableSupplier({
  ...
  onStatsUpdate?: (stats: SuplierStats) => void;
}) {
  ...
  const fetchSuppliers = async (page = currentPage) => {
    try {
      ...
      const totalContact = res.data.filter(
        (s) => s.email || s.phone_number
      ).length;

      onStatsUpdate?.({
        totalSuplier: res.pagination.total,  
        totalActive: res.data.length,         
        totalContact,                          
      });
      ...
      onStatsUpdate?.({ totalSuplier: 0, totalActive: 0, totalContact: 0 });
    } finally {
      setLoading(false);
    }
  };
