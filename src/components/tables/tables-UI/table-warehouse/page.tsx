...
export default function TableWarehouse({
  ...
  const fetchWarehouses = async (page = currentPage) => {
    try {
      ...
      // const uniqueTypes = new Set(res.data.map((w) => w.type).filter(Boolean));

      onStatsUpdate?.({
        totalWarehouse: res.pagination.total,   // total semua warehouse (dari pagination)
      ...
      onStatsUpdate?.({ totalWarehouse: 0, totalActive: 0, totalSearch: 0 });
    } finally {
      setLoading(false);
    }
  };
  ...
  // ✅ Update totalSearch saat hasil filter berubah
  useEffect(() => {
    if (warehouses.length === 0) return;
    onStatsUpdate?.({
      totalWarehouse: totalItems,
      totalActive: warehouses.length,
      totalSearch: filteredWarehouses.length,
    });
  }, [filteredWarehouses, totalItems, warehouses]);
